...
# Description: Use ttk Styling for Buttons and Labels
# Source URL: https://coderslegacy.com/python/tkinter-ttk-style/
# Source URL: https://www.tcl.tk/man/tcl/TkCmd/ttk_button.html
...
class FlashcardApp(tk.Tk):
    def __init__(self):
        ...
        self.style.map(
            ...
            background=[("pressed", "!disabled", "#5096f2"), ("active", "#dce9fa")],
        )

        # Create Home Button Style
        self.style.configure(
            "Home.TButton",
            foreground="black",
            background="#b2cff7",
        ...
        # Create Submit/Add/Save Button Styles
        self.style.configure(
            "Positive.TButton",
            foreground="black",
            background="#92DDC7",
        ...
        self.style.map(
            "Positive.TButton",
            foreground=[("pressed", "#14453D"), ("active", "#14453D")],
            background=[("pressed", "!disabled", "#37A989"), ("active", "#E0F5EF")],
        )
        ...
        self.style.map(
            "Delete.TButton",
            foreground=[("pressed", "#88070E"), ("active", "#88070E")],
            background=[("pressed", "!disabled", "#F7646B"), ("active", "#FDD8DA")],
        )

        # Create Show Definition Button Styles
        self.style.configure(
            "Show.TButton",
            foreground="black",
            background="#C2BDDB",
        ...
        self.style.map(
            "Show.TButton",
            foreground=[("pressed", "#423A69"), ("active", "#423A69")],
            background=[("pressed", "!disabled", "#796EAF"), ("active", "#CECAE2")],
        )
...
class HomePage(tk.Frame):
    def __init__(self, master):
        ...
        self.welcome_label = tk.Label(
            self, text="Welcome!", font=("Arial", 30, "bold"), pady=10
        )
        ...
        self.view_cards_button = ttk.Button(
            ...
            style="Custom.TButton",
            command=lambda: self.check_deck(master, ViewCardsPage),
        )
        self.import_cards_button = ttk.Button(
            self,
            text="Import Flashcards",
            style="Custom.TButton",
            command=lambda: master.switch_frame(ImportCardsPage),
        )
        self.export_cards_button = ttk.Button(
            self,
            text="Export Flashcards",
            style="Custom.TButton",
            command=lambda: self.check_deck(master, ExportCardsPage),
        )
    ...
    def check_deck(self, master, page):
        ...
        if deck is True:
            ...
            messagebox.showinfo(
                title="Blank Deck",
                message="You have not added any flashcards to your deck.",
            )
...
class CreateCardsPage(tk.Frame):
    def __init__(self, master):
        ...
        self.create_cards_label = tk.Label(
            self, text="Create Cards", font=("Arial", 30, "bold"), pady=10
        )

        # Initialize Flashcard Term/Def Labels & Text Fields
        self.term_label = tk.Label(
            self, text="Enter the Flashcard Term:", font=("Arial", 18)
        )
        self.term_entry = tk.Entry(self, width=40, font=("Arial", 15))

        self.def_label = tk.Label(
            self, text="Enter the Flashcard Definition:", font=("Arial", 18)
        )
        ...
        self.generate_def_button = ttk.Button(
            self,
            text="Auto-Generate Definition",
            style="Definition.TButton",
            command=self.generate_def,
        )
        self.generate_def_label = tk.Label(self, text="")
        ...
        self.create_cards_label.grid(column=0, row=0, padx=10, pady=5, columnspan=2)
        ...
        self.submit_button.grid(
            column=1, row=6, padx=5, pady=5, columnspan=1, sticky="w"
        )
        self.home_button.grid(column=0, row=7, padx=5, pady=5, columnspan=1)
    ...
    def generate_def(self):
        ...
        if not term:
            messagebox.showerror(
                title="Blank Term", message=f"Term field cannot be blank."
            )
            return
        else:
    ...
    def show_generate_def_label(self, event):
        ...
        self.generate_def_label.grid(row=1, column=1, sticky="e")
    ...
    def create_card_gui(self):
        ...
        if not definition:
            ...
            if added:
                added_confirmation = tk.Label(self, text="Card Added", fg="#32CD32")
                added_confirmation.grid(column=0, row=6, pady=5)
                self.term_entry.delete(0, "end")
                self.def_text.delete(1.0, "end")
                added_confirmation.after(4000, lambda: added_confirmation.destroy())
            else:
                messagebox.showerror(
...
class StudyCardsPage(tk.Frame):
    def __init__(self, master):
        ...
        self.study_cards_label = tk.Label(
            self, text="Study Cards", font=("Arial", 25, "bold")
        )
        ...
        self.start_study_button = ttk.Button(
            ...
            text="Start Studying!",
            style="Positive.TButton",
            command=lambda: self.show_card_display(master),
        )

        # Bind Start Study Button to the user's click so the button disappears
        self.start_study_button.bind("<ButtonRelease-1>", self.forget_study_button)
        ...
        self.home_button.grid(column=0, row=3, pady=10)
    ...
    def show_def_button(self, master, term_tuple):
        ...
        """
        show_def_button = ttk.Button(
            self.card_display,
            text="Show Definition",
            style="Show.TButton",
            command=lambda: self.show_definition(master, term_tuple),
        )
        show_def_button.grid(column=0, row=1, sticky="se", padx=10, pady=10)

    def show_next_button(self, master):
        """
        Initializes and Displays Next Button.
        """
        show_next_button = ttk.Button(
            self.card_display,
            text="Next Card",
            style="Show.TButton",
            command=lambda: self.study(master),
        )
        show_next_button.grid(column=1, row=1, sticky="sw", padx=10, pady=10)
    ...
    def show_definition(self, master, term_tuple):
        ...
        show_term_button = ttk.Button(
            self.card_display,
            text="Show Term",
            style="Show.TButton",
            command=lambda: self.show_term(master, term_tuple),
        )
        show_term_button.grid(column=0, row=1, sticky="se", padx=10, pady=10)
        self.show_next_button(master)
    ...
    def repeat_deck(self, master):
        ...
        repeat = messagebox.askyesno(
            ...
            message="You have completed studying all your cards! Would you like to play again?",
        )
        if repeat:
    ...
    def forget_study_button(self, event):
        ...
        """
        self.start_study_button.grid_forget()
...
class ViewCardsPage(tk.Frame):
    def __init__(self, master):
        ...
        self.view_cards_label = tk.Label(
            self, text="View Cards", font=("Arial", 25, "bold")
        )
        ...
        self.crud_frame = tk.LabelFrame(
            self, text="Currently Selected", font=("Arial", 20, "bold")
        )

        # Create labels/fields for CRUD operations within the crud_frame
        self.selected_term_label = tk.Label(
            self.crud_frame, font=("Arial", 16, "bold"), text="Selected Term"
        )
        self.selected_term_entry = tk.Entry(
            self.crud_frame, width=40, font=("Arial", 14)
        )
        self.selected_def_label = tk.Label(
            self.crud_frame, font=("Arial", 16, "bold"), text="Selected Definition"
        )
        ...
        self.edit_card_button = ttk.Button(
            self.crud_frame,
            text="Save Selected Card",
            style="Positive.TButton",
        ...
        self.view_cards_label.grid(column=0, row=0, pady=10, columnspan=2)
        self.card_display.grid(column=0, row=1, pady=10, columnspan=2)
        self.tree = self.initialize_treeview()
        self.fill_treeview()
        self.crud_frame.grid(column=0, row=2, pady=5, columnspan=2)
        self.delete_all_button.grid(column=0, row=4, pady=10)
        self.home_button.grid(column=1, row=4, pady=10)
        ...
        self.selected_term_label.grid(row=0, column=0, padx=10, pady=10, sticky="w")
        self.selected_term_entry.grid(row=0, column=1, padx=10, pady=10, sticky="w")
        self.selected_def_label.grid(row=1, column=0, padx=10, pady=10, sticky="w")
        self.selected_def_text.grid(row=1, column=1, padx=5, pady=5, sticky="e")
        self.delete_card_button.grid(row=2, column=0, padx=5, pady=5, sticky="e")
        self.edit_card_button.grid(row=2, column=1, padx=5, pady=5, sticky="w")
    ...
    def fill_treeview(self):
        ...
        self.tree.column("Term", width=150, minwidth=35)
        self.tree.column("Definition", width=500, minwidth=35)
        ...
        for i in range(len(self.cards_dict)):
            self.tree.insert(
                ...
                iid=i,
                text="Terms",
                values=(i + 1, self.cards_dict[i][0], self.cards_dict[i][1]),
            )
    ...
    def edit_selected(self):
        ...
        if repeat > 1:
            ...
            self.tree.item(
                selected_obj, text="", values=(selected_row[0], new_term, new_def)
            )
    ...
    def delete_selected(self):
        ...
        delete_selected_warning = messagebox.askyesno(
            title="Warning",
            message="Deleting a flashcard cannot be undone. Do you still want to proceed?",
        )
        if delete_selected_warning:
            ...
            self.selected_term_entry.delete(0, "end")
            self.selected_def_text.delete("1.0", "end")
            deleted_confirmation = tk.Label(self, text="Card Deleted", fg="#32CD32")
            deleted_confirmation.grid(column=0, row=3, pady=3)
            deleted_confirmation.after(4000, lambda: deleted_confirmation.destroy())
    ...
    def delete_all(self):
        ...
        delete_all_warning = messagebox.askyesno(
            title="Warning",
            message="You are about to delete ALL of your flashcards. This cannot be undone. Do you still want to proceed?",
        )
...
class ImportCardsPage(tk.Frame):
    def __init__(self, master):
        ...
        self.import_cards_label = tk.Label(
            self,
            text="Import Cards from a CSV File",
            font=("Arial", 25, "bold"),
            pady=10,
        )
        ...
        self.import_cards_instructions = tk.Label(
            self,
            text="(1) Click on 'Import Now' to open a file browser. \n(2) Select a valid .csv file consisting of two comma-separated columns (term and definition). \n(3) Click 'Open' to import.",
            pady=10,
            font=("Arial", 16),
            justify="left",
        )
        ...
        self.image_frame1 = tk.LabelFrame(
            self,
            text="Example 1: Format of text .csv file",
            font=("Arial", 15, "bold"),
            width=400,
            height=200,
        )
        self.sample_plaintext = ImageTk.PhotoImage(Image.open("plaintextcsv.png"))
        self.image1 = tk.Label(self.image_frame1, image=self.sample_plaintext)
        self.image_frame2 = tk.LabelFrame(
            self,
            text="Example 2: Format of spreadsheet .csv file",
            font=("Arial", 15, "bold"),
            width=400,
            height=200,
        )
        self.sample_spreadsheet = ImageTk.PhotoImage(Image.open("spreadsheetcsv.png"))
        self.image2 = tk.Label(self.image_frame2, image=self.sample_spreadsheet)

        # Initialize Buttons
        self.import_now_button = ttk.Button(
            self,
            text="Import Cards Now",
            style="Positive.TButton",
            command=self.import_file_dialog,
        )
        self.home_button = ttk.Button(
            self,
            text="Home Page",
        ...
        self.image_frame1.grid(column=0, row=3, padx=10, pady=10)
        self.image_frame2.grid(column=0, row=4, padx=10, pady=10)
        self.import_now_button.grid(column=0, row=5, padx=10, pady=10)
        self.home_button.grid(column=0, row=6, padx=10, pady=10)
        self.image1.pack()
        self.image2.pack()

    def import_file_dialog(self):
        self.filename = filedialog.askopenfilename(
            initialdir="/CS-361/FlashcardMaker",
        ...
        if imported == "successful":
            added = messagebox.askyesno(
                title="Confirmation",
                message="Your flashcards have been added. Go to ViewCards Page?",
            )
            if added:
            ...
            added = messagebox.showerror(
                title="Import Error",
                message="Your flashcards could not be imported. Invalid .csv format.",
            )
        else:
            added = messagebox.showerror(
                title="Import Error",
                message=f"Your flashcards could not be imported. {imported} is a duplicate term.",
            )
...
class ExportCardsPage(tk.Frame):
    def __init__(self, master):
        ...
        self.export_cards_label = tk.Label(
            self, text="Export Cards to a CSV File", font=("Arial", 25, "bold"), pady=10
        )

        # Initialize Instructions
        self.export_cards_instructions = tk.Label(
            self,
            text='(1) Click on "Export Now" to open the file browser. \n(2) Navigate to desired file destination. \n(3) Enter your desired filename and click "Save" to export.',
            font=("Arial", 16),
            justify="left",
            pady=15,
        )
        ...
        # Initialize Buttons
        self.export_now_button = ttk.Button(
            self,
            text="Export Cards Now",
            style="Positive.TButton",
            command=self.export_file_dialog,
        )
        self.home_button = ttk.Button(
            self,
            text="Home Page",
        ...
        self.export_now_button.grid(column=0, row=3, padx=10, pady=10)
        self.home_button.grid(column=0, row=4, padx=10, pady=10)
    ...
    def export_file_dialog(self):
        ...
        added = messagebox.showinfo(
            title="Confirmation",
            message=f"Your flashcards have been exported as {self.filename}. ",
        )
